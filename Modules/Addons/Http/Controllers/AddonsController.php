...
use DB, Artisan, Cache, Exception;
...
class AddonsController extends Controller
{
    ...
    public function upload(UploadAddonRequest $request)
    {
        try {
            ...
            $addon = AddonManager::upload($request->attachment);
            if (!$addon) {
                return back()->with(['AddonStatus'=> 'fail', 'AddonMessage' => __('Required files not found in the compressed addon file.')])->withInput();
            }
            ...
            AddonManager::migrateAndSeed($addon->get('name'));
