        ...
        <div class="box box-info">
            ...
            <form action="{{ url(\Config::get('adminPrefix').'/settings') }}" method="post" class="form-horizontal" enctype="multipart/form-data" id="general_settings_form">
                ...
                <!-- box-body -->
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-6">
                      <select class="form-control has_captcha" name="has_captcha" id="has_captcha">
                          <option value='login' {{ $result['has_captcha'] == 'login' ? 'selected':""}}>Login</option>
                          <option value='registration' {{ $result['has_captcha'] == 'registration' ? 'selected':""}}>Registration</option>
                          <option value='login_and_registration' {{ $result['has_captcha'] == 'login_and_registration' ? 'selected':""}}>Login And Registration</option>
                  ...
                  <div class="form-group">
                    <label for="inputEmail3" class="col-sm-4 control-label">Allowed Wallets</label>
                    <div class="col-sm-6">
                      <select class="form-control" name="allowed_wallets[]" id="allowed_wallets" multiple>
                          @foreach ($allowedWallets as $key => $value)
                            <option value='{{ $key }}'>{{ $value }}</option>
                          @endforeach
                      </select>
                      <small class="form-text text-muted"><strong>*This currency wallets will be generated during registration besides default one</strong></small>
                    </div>
                  </div>
                  <!-- Default Language -->
        					<div class="form-group">
                        ...
                        <button type="submit" class="btn btn-theme pull-right" id="general-settings-submit">
...
@push('extra_body_scripts')
  ...
  <script type="text/javascript">
      ...
      function updateSideBarCompanySmallLogo(file)
      {
          if (file.name.match(/.(png|jpg|jpeg|gif|bmp)$/i))
          {
            $.ajax(
            {
                ...
                type:'POST',
                url: SITE_URL+"/"+ADMIN_PREFIX+"/settings/update-sidebar-company-logo",
      ...
      $(window).on('load', function()
      {
          ...
          $('#allowed_wallets').selectpicker();

          let selectedCryptoCurrencies = '{{ $result['allowed_wallets'] }}';

          if (selectedCryptoCurrencies != 'none')
          {
              $.each(selectedCryptoCurrencies.split(","), function(i,e)
              {
                  $("#allowed_wallets option[value='" + e + "']").prop("selected", true);
                  $('#allowed_wallets').selectpicker('refresh');
              });
          }
      });
      ...
      $(document).on('click','.remove_img_preview_site_logo', function()
      {
          ...
          if(logo)
          {
            $.ajax(
            {
              ...
              type : "POST",
              url : SITE_URL+"/"+ADMIN_PREFIX+"/settings/delete-logo",
              data: {
                'logo' : logo,
              },
              dataType : 'json',
      ...
      $(document).on('click','.remove_fav_preview', function()
      {
          ...
          if(favicon)
          {
            $.ajax(
            {
              ...
              type : "POST",
              url : SITE_URL+"/"+ADMIN_PREFIX+"/settings/delete-favicon",
              data: {
                'favicon' : favicon,
              },
              dataType : 'json',
