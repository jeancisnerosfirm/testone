				    ...
				    <tr>
					  <td>
					   <table>
						  ...
						  <tr>
							<td style="font-size:15px; color:#4e5c6e; line-height:22px;">{{ optional($transaction->currency)->code }}</td>
						  </tr>
						  </table>
					  </td>
					  </tr>
					  ...
					  <td>
					   <table style="margin-top:20px; width:300px;">
						  ...
						  <tr>
							...
							<td style="font-size:15px; color:#4e5c6e; text-align:right;">{{ moneyFormat(optional($transaction->currency)->symbol, formatNumber($transaction->subtotal, $transaction->currency_id)) }}</td>
						  </tr>
						  	...
						  	@if (abs($transaction->total) - abs($transaction->subtotal) > 0)
								<tr style="padding-bottom:10px;">
									...
									<td style="font-size:15px; color:#4e5c6e; text-align:right;">{{ moneyFormat(optional($transaction->currency)->symbol, formatNumber($transaction->charge_percentage + $transaction->charge_fixed, $transaction->currency_id)) }}</td>
								</tr>
							@endif
						  ...
						  <tr>
							...
							<td style="font-size:15px; color:#4e5c6e; text-align:right; font-weight:bold;">{{ moneyFormat(optional($transaction->currency)->symbol, formatNumber($transaction->subtotal + $transaction->charge_percentage + $transaction->charge_fixed, $transaction->currency_id)) }}</td>
						  </tr>
						  </table>
					    </td>
					  </tr>
					...
					<tr>
					  <td>
					   <table style="margin-top:20px; width:300px;">
						  ...
						  <tr>
							<td style="font-size:15px; color:#000000; font-weight:bold;">@lang('message.dashboard.left-table.total')</td>
							<td style="font-size:15px; color:#4e5c6e; text-align:right; font-weight:bold;">{{ moneyFormat(optional($transaction->currency)->symbol, formatNumber($transaction->subtotal, $transaction->currency_id)) }}</td>
						  </tr>
						  </table>
					    </td>
					  </tr>
