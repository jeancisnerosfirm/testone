                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p class="mb20">
                                ...
                                <span class="font-weight-600 text-uppercase">{{ isset($transInfo['payout_setting']->paymentMethod->name) && !empty($transInfo['payout_setting']->paymentMethod->name) ? strtolower($transInfo['payout_setting']->paymentMethod->name) : '' }}</span>
                            </p>

                            @if ( isset($transInfo['payout_setting']->paymentMethod->name) && $transInfo['payout_setting']->paymentMethod->name == 'Bank')
                            ...
                            <div class="row m-0 mt-2 justify-content-between">
                                ...
                                <div>
                                   {{  moneyFormat($transInfo['currSymbol'], formatNumber($transInfo['amount'], $transInfo['currency_id']))  }}
                                </div>
                            </div>
                            ...
                            <div class="row justify-content-between m-0 mt-2">
                                ...
                                <div>{{  moneyFormat($transInfo['currSymbol'], formatNumber($transInfo['fee'], $transInfo['currency_id'])) }}</div>
                            </div>
                            <hr class="my-2" />
                            <div class="row justify-content-between m-0">
                                <div><strong>@lang('message.dashboard.confirmation.total')</strong></div>
                                <div><strong>{{  moneyFormat($transInfo['currSymbol'], formatNumber($transInfo['totalAmount'], $transInfo['currency_id'])) }}</strong></div>
                            </div>
