				...
				<div class="card">
					...
					<div class="card-body">
						<p class="mb20">@lang('message.dashboard.exchange.confirm.exchanging') <strong>{{ $fromCurrency->code }}</strong>
							@lang('message.dashboard.exchange.confirm.of') <strong>{{ isset($transInfo['defaultAmnt']) ? formatNumber($transInfo['defaultAmnt'], $transInfo['currency_id']) : 0.00 }}</strong>
							@lang('message.dashboard.exchange.confirm.equivalent-to') <strong>{{ isset($transInfo['finalAmount']) ? formatNumber($transInfo['finalAmount'], $transInfo['currency_id']) : 0.00 }} {{ $transInfo['currCode'] }}</strong>
						</p>
						<p>@lang('message.dashboard.exchange.confirm.exchange-rate'):  &nbsp;<strong>1 {{$fromCurrency->code}} </strong>= <strong>
							{{ formatNumber($transInfo['dCurrencyRate'], $transInfo['currency_id']) }} {{ $transInfo['currCode'] }}</strong></p>
						...
						<div class="mt-4">
							<div class="row m-0 justify-content-between mt-2">
								...
								<div>{{  moneyFormat($fromCurrency->symbol, isset($transInfo['defaultAmnt']) ? formatNumber($transInfo['defaultAmnt'], $transInfo['currency_id']) : 0.00) }}</div>
							</div>

							<div class="row justify-content-between m-0 mt-2">
								<div>@lang('message.dashboard.confirmation.fee')</div>
								<div>{{  moneyFormat($fromCurrency->symbol, isset($transInfo['fee']) ? formatNumber($transInfo['fee'],  $transInfo['currency_id']) : 0.00) }}</div>
							</div>
							<hr class="my-2" />
							<div class="row m-0 justify-content-between">
								<div><strong>@lang('message.dashboard.confirmation.total')</strong></div>
								<div><strong>{{  moneyFormat($fromCurrency->symbol, isset($transInfo['totalAmount']) ? formatNumber($transInfo['totalAmount'],  $transInfo['currency_id']) : 0.00) }}</strong></div>
							</div>
						</div>
