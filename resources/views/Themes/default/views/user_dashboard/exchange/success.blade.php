			...
			<div class="col-lg-6 col-xs-12">
				<div class="card">
					...
					<div class="card-body">
						...
						<strong>{{  moneyFormat(optional($fromWallet->currency)->symbol, isset($transInfo['defaultAmnt']) ? formatNumber($transInfo['defaultAmnt'], $transInfo['currency_id']) : 0.00) }}</strong>
						@lang('message.dashboard.exchange.confirm.has-exchanged-to') <strong>
							{{  moneyFormat($transInfo['currSymbol'], isset($transInfo['finalAmount']) ? formatNumber($transInfo['finalAmount'], $transInfo['currency_id']) : 0.00) }}
						</strong><br/>@lang('message.dashboard.exchange.confirm.exchange-rate'):<strong> 1 {{ optional($fromWallet->currency)->code }}</strong> = <strong>{{ formatNumber($transInfo['dCurrencyRate'], $transInfo['currency_id']) }}</strong>
