...
class Transfer extends Model
{
    ...
    public function mailToSender($transfer, $arr)
    {
        if (checkAppMailEnvironment())
        {
            ...
            $sender_info           = $this->helper->getEmailOrSmsTemplate(1, 'email', settings('default_language'));
    ...
    public function mailToReceiver($transfer, $arr)
    {
        if (checkAppMailEnvironment())
        {
            ...
            $receiver_info           = $this->helper->getEmailOrSmsTemplate(2, 'email', settings('default_language'));
            if (isset($arr['userInfo']))
            {
                ...
                $receiver_msg = str_replace('{soft_name}', settings('name'), $receiver_msg);
    ...
    public function smsToSender($transfer, $arr)
    {
        if (checkAppSmsEnvironment())
        {
            if (!empty($transfer->sender->carrierCode) && !empty($transfer->sender->phone))
            {
                ...
                $senderSmsInfo            = $this->helper->getEmailOrSmsTemplate(1, 'sms', settings('default_language'));
    ...
    public function smsToReceiver($transfer, $arr)
    {
        if (checkAppSmsEnvironment())
        {
            if (!empty($transfer->receiver->carrierCode) && !empty($transfer->receiver->phone))
            {
                ...
                $receiverSmsInfo            = $this->helper->getEmailOrSmsTemplate(2, 'sms', settings('default_language'));
