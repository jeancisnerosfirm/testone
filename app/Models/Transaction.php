...
class Transaction extends Model
{
    ...
    public static $transactionTypes = [Deposit, Withdrawal, Transferred, Received, Exchange_From, Exchange_To, Request_From, Request_To, Payment_Sent, Payment_Received];
    ...
    public function __construct()
    {
        ...
        if (module('CryptoExchange') ) {
            array_push(self::$transactionTypes, Crypto_Exchange, Crypto_Buy, Crypto_Sell);
        }
    ...
    public function getTransactions($from, $to, $type, $wallet, $status)
    {
        ...
        if (!empty($type) && $type != 'all')
        {
            ...
            else
            {
                ...
                } elseif (config('referral.is_active') && $type == 'referral_award') {
                    $whereInCondition = [Referral_Award];
                }
                elseif (module('CryptoExchange') && $type == 'crypto_exchange' ) {
                    $whereInCondition = [Crypto_Exchange];
                } elseif (module('CryptoExchange') && $type == 'crypto_sell' ) {
                    $whereInCondition = [Crypto_Sell];
                } elseif (module('CryptoExchange') && $type == 'crypto_buy' ) {
                    $whereInCondition = [Crypto_Buy];
                }
    ...
    public function getTransactionDetails($tr_id, $user_id)
    {
        ...
        //formatNumber - starts
        $transaction->total = moneyFormat($transaction->currency->symbol, formatNumber($transaction->total, $transaction->currency_id));

        $transaction->subtotal = moneyFormat($transaction->currency->symbol, formatNumber($transaction->subtotal, $transaction->currency_id));

        if ($transaction->currency->type != 'fiat')
        {
            $transaction->totalFees = (($transaction->charge_percentage == 0) && ($transaction->charge_fixed == 0)) ? moneyFormat($transaction->currency->symbol, formatNumber(0, $transaction->currency_id)) :
            moneyFormat($transaction->currency->symbol, formatNumber($transaction->charge_fixed, $transaction->currency_id));
        }
        else
        {
            $transaction->totalFees = (($transaction->charge_percentage == 0) && ($transaction->charge_fixed == 0)) ? moneyFormat($transaction->currency->symbol, formatNumber(0, $transaction->currency_id)) : moneyFormat($transaction->currency->symbol, formatNumber($transaction->charge_percentage + $transaction->charge_fixed, $transaction->currency_id));
        }
        //formatNumber - ends
