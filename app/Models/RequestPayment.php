...
class RequestPayment extends Model
{
    ...
    public function mailToRegisteredRequestAcceptor($requestPayment, $arr)
    {
        ...
        if (checkAppMailEnvironment())
        {
            ...
            $req_create_temp       = $this->helper->getEmailOrSmsTemplate(4, 'email', settings('default_language'));
            ...
            $req_create_msg = str_replace('{soft_name}', settings('name'), $req_create_msg);
            try {
    ...
    public function smsToRequestAcceptor($requestPayment, $arr)
    {
        if (checkAppSmsEnvironment())
        {
            if (isset($arr['userInfo']))
            {
                ...
                $reqSmsTempInfo  = $this->helper->getEmailOrSmsTemplate(4, 'sms', settings('default_language'));
    ...
    public function onlySmsToRequestCreatorOnRequestAccept($arr, $requestPayment)
    {
        if (checkAppSmsEnvironment())
        {
            if (!empty($requestPayment->user->carrierCode) && !empty($requestPayment->user->phone))
            {
                ...
                $reqPaymentAcceptSmsTempInfo = $this->helper->getEmailOrSmsTemplate(5, 'sms', settings('default_language'));
    ...
    public function onlyEmailToRequestCreatorOnRequestAccept($arr, $requestPayment)
    {
        if (checkAppMailEnvironment())
        {
            ...
            $rp_accept_temp        = $this->helper->getEmailOrSmsTemplate(5, 'email', settings('default_language'));
            ...
            $rp_msg = str_replace('{soft_name}', settings('name'), $rp_msg);
