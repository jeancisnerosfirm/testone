...
class TransactionsExport implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithStyles
{
    ...
    public function user($transaction)
    {
        ...
        if (in_array($transaction->transaction_type_id, $transactionTypes['sent'])) {
            $user = isset($transaction->user->first_name) && !empty($transaction->user->first_name) ? $transaction->user->first_name . ' ' . $transaction->user->last_name : ((isset($transaction->crypto_exchange)) ? $transaction->crypto_exchange->email_phone : '-') ;
