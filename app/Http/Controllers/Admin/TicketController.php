...
class TicketController extends Controller
{
    ...
    public function index(TicketsDataTable $dataTable)
    {
        ...
        $data['summary'] = DB::select(DB::raw("SELECT COUNT(tickets.ticket_status_id) as total_status, ticket_statuses.name,ticket_statuses.id, ticket_statuses.color as color FROM tickets RIGHT JOIN ticket_statuses ON ticket_statuses.id = tickets.ticket_status_id GROUP BY ticket_statuses.id"));
        ...
        $data['getName']  = $this->ticket->getTicketsUserName($user);
    ...
    public function store(Request $request)
    {
        ...
        $ticket->code             = 'TIC-' . strtoupper(Str::random(6));
        ...
        return redirect()->intended(Config::get('adminPrefix').'/tickets/list');
    }
    ...
    public function edit($id)
    {
        ...
        $data['ticket'] = $ticket = Ticket::find($id);
        $data['user'] = User::where(['id' => $ticket->user_id, 'status' => 'Active'])->first();
        $data['admins'] = Admin::where(['status' => 'Active'])->get();
        $data['ticket_statuses'] = TicketStatus::get();
        if(!g_c_v() && a_te_c_v()) {
            Session::flush();
