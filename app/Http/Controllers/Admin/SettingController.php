...
use Config, Validator, Session, Cache, View, Mail, DB, Exception, ZipArchive;
...
class SettingController extends Controller
{
    ...
    public function adminSecuritySettings(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {  
            ...
            if (! empty($request->get('admin_access_ips'))) {
                ...
                $this->helper->one_time_message('error', 'You have to add at least one IP before update.');
                return redirect(Config::get('adminPrefix').'/settings/admin-security-settings');
            }
            ...
            if (!empty($request->get('admin_url_prefix'))) {
                ...
                $this->helper->one_time_message('error', 'Admin Prefix not set');
                return redirect(Config::get('adminPrefix').'/settings/admin-security-settings');
            }

            $this->helper->one_time_message('success', 'Admin security settings updated successfully.');
            return redirect(Config::get('adminPrefix').'/settings/admin-security-settings');
        }
    ...
    {
        if (isset($request->theme) && Theme::has($request->theme)) {
            changeEnvironmentVariable('THEME', $request->theme);
            $this->helper->one_time_message('success', 'Theme successfully activated.');
            return redirect(Config::get('adminPrefix').'/settings/themes');
        }
        $this->helper->one_time_message('error', 'Opps, something went wrong, try again.');
        return redirect(Config::get('adminPrefix').'/settings/themes');
    }
    ...
    public function currencyConversionRateApi(Request $request)
    {
        if ($request->isMethod('post')) {
            $rules = [
                'currency_converter_api_key' => 'required_without_all:exchange_rate_api_key',
                'exchange_rate_api_key' => 'required_without_all:currency_converter_api_key',
            ];
            $messages = [
                'currency_converter_api_key.required_without_all' => 'At least api key one is required',
                'exchange_rate_api_key.required_without_all' => 'At least one api key is required',
            ];
            ...
            if ($validator->fails()) {
                ...
                Cache::forget(config('cache.prefix') . '-settings');
                $this->helper->one_time_message('success', 'Currency conversion rate api updated successfully');
