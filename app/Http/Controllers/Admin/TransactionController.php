...
class TransactionController extends Controller
{
    ...
    public function transactionPdf()
    {
        ...
        $data['transactions'] = $this->transaction->getTransactionsList($from, $to, $status, $currency, $type, $user)->orderBy('transactions.id', 'desc')->take(1100)->get(); //mdf problem, so, i have set take(1100)
    ...
    public function edit($id)
    {
        ...
        if (module('CryptoExchange')) {
            $crypto_relation = 'crypto_exchange:id,receiver_address,file_name,payment_details,exchange_rate,to_currency,email_phone,verification_via';
            array_push($relation, $crypto_relation);
        }

        $data['transaction'] = $transaction = Transaction::with($relation)->find($id);

        if (module('CryptoExchange') && !empty($transaction->crypto_exchange)){

            if (isset($transaction->crypto_exchange->receiver_address)) {
                $data['receiver_address'] = $transaction->crypto_exchange->receiver_address;
            }

            if (isset($transaction->crypto_exchange->file_name)) {
                $data['file_name'] = $transaction->crypto_exchange->file_name;
            }

            if (isset($transaction->crypto_exchange->payment_details)) {
                $data['payment_details'] = $transaction->crypto_exchange->payment_details;
            }

            if (isset($transaction->crypto_exchange->exchange_rate)) {
                $data['exchange_rate'] = $transaction->crypto_exchange->exchange_rate;
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->type == 'Crypto Exchange' || $request->type == 'Crypto Buy' || $request->type == 'Crypto Sell') {
            ...
            $cryptoSuccessMsg = str_replace('{uuid}', $cryptoExchange->uuid, $cryptoSuccessMsg);
            $cryptoSuccessMsg = str_replace('{status}', $cryptoExchange->status, $cryptoSuccessMsg);
            $cryptoSuccessMsg = str_replace('{amount}', moneyFormat($cryptoExchange->toCurrency->symbol, formatNumber($cryptoExchange->get_amount, $cryptoExchange->to_currency)), $cryptoSuccessMsg);
            $cryptoSuccessMsg = str_replace('{added/subtracted}', 'added', $cryptoSuccessMsg);
            $cryptoSuccessMsg = str_replace('{from/to}', 'to', $cryptoSuccessMsg);
            $cryptoSuccessMsg = str_replace('{soft_name}', Session::get('name'), $cryptoSuccessMsg);
            ...
            if (checkAppSmsEnvironment()) {
                ...
                $cryptoSuccessSmsMsg = str_replace('{uuid}', $cryptoExchange->uuid, $cryptoSuccessSmsMsg);
                $cryptoSuccessSmsMsg = str_replace('{status}', $cryptoExchange->status, $cryptoSuccessSmsMsg);
                $cryptoSuccessSmsMsg = str_replace('{amount}', moneyFormat($cryptoExchange->toCurrency->symbol, formatNumber($cryptoExchange->get_amount, $cryptoExchange->to_currency)),
                    $cryptoSuccessSmsMsg);
                $cryptoSuccessSmsMsg = str_replace('{added/subtracted}', 'added', $cryptoSuccessSmsMsg);
                $cryptoSuccessSmsMsg = str_replace('{from/to}', 'to', $cryptoSuccessSmsMsg);
