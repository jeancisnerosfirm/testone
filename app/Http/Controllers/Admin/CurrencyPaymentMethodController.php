...
class CurrencyPaymentMethodController extends Controller
{
    ...
    public function paymentMethodList($tab, $id)
    {
        ...
        $data['currencyList'] = (new Currency())->getAllCurrencies(['status' => 'Active', 'type' => $type], ['id','name']);
    ...
    public function addBank(Request $request)
    {
        ...
        if ($validation->passes()) {
            ...
            return response()->json([
                'status'  => true,
                'message' => __('Bank has been added successfully.'),
            ]);
        }
        else
        {
    ...
    public function getCpmId(Request $request)
    {
        ...
        if ($cpm) {
            ...
            $data['status'] = false;
            $data['cpmId']  = __('Currency payment method not found.');
        }
        return $data;
    }
    ...
    public function updateBank(Request $request)
    {
        ...
        if ($validation->passes()) {
            ...
            return response()->json([
                'status'  => true,
                'message' => __('The Bank is successfully updated.'),
            ]);
        } else {
            return response()->json([
                'status'  => false,
        ...
        if (!empty($bank)) {
            ...
            if (!empty($currencyPaymentMethod)) {
                ...
                return response()->json([
                    'status'  => true,
                    'type'    => 'success',
                    'message' => __('The bank has been deleted successfully.'),
                ]);
            } else {
                return response()->json([
                    'status'  => false,
                    'type'    => 'error',
                    'message' => __('The Bank could not be deleted.'),
                ]);
            }
        } else {
            return response()->json([
    ...
    public function addMobileMoney(Request $request)
    {
        ...
        if ($validation->passes()) {
            try {
                ...
                return response()->json([
                    'status'  => 200,
                    'message' => __('MobileMoney has been added successfully.'),
                ]);
    ...
    public function updateMobileMoney(Request $request)
    {
        ...
        if ($validation->passes()) {
            ...
            try {
                ...
                return response()->json([
                    'status'  => 200,
                    'message' => __('MobileMoney has been updated successfully.'),
                ]);
            ...
            try {
                ...
                if ($currencyPaymentMethod->status == 200) {
                    ...
                    return response()->json([
                        'status'  => 200,
                        'type'    => 'success',
                        'message' => __('MobileMoney has been deleted Successfully.'),
                    ]);
                } else {
                    return response()->json([
                        'status'  => false,
                        'type'    => 'error',
                        'message' => __('The MobileMoney could not be deleted.'),
                    ]);
                }
            } catch (Exception $e) {
                DB::rollback();
