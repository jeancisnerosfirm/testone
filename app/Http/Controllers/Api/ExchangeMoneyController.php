...
class ExchangeMoneyController extends Controller
{
    ...
    public function getCurrenciesExchangeRate(Request $request)
    {
        ...
        if (!empty($toWalletCurrency)) {
            if ($toWalletCurrency->exchange_from == "api" && settings('exchange_enabled_api') != 'Disabled' && ((settings('exchange_enabled_api') == 'currency_converter_api_key' && !empty(settings('currency_converter_api_key')))  || (settings('exchange_enabled_api') == 'exchange_rate_api_key' && !empty(settings('exchange_rate_api_key')))) ){
