...
use Artisan, Validator, Session, Hash, Auth, DB;
...
class CustomerController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $data['wallets'] = Wallet::with('currency:id,type,logo,code,status')->where(['user_id' => Auth::user()->id])->orderBy('balance', 'ASC')->get(['id', 'currency_id', 'balance', 'is_default']);
    ...
    public function ajaxTwoFa(Request $request)
    {
        if (auth()->user()->user_detail->two_step_verification_type !== $request->two_step_verification_type)
        {
            ...
            {
                //sms
                $message = $six_digit_random_number . ' is your ' . settings('name') . ' 2-factor verification code. ';
    ...
    public function updatePersonalId(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $this->validate($request, [
                ...
                'identity_file' => 'mimes:pdf,png,jpg,jpeg,gif,bmp|max:10000',
            ]);
    ...
    public function personalAddress()
    {
        ...
        $data['user'] = User::find(Auth::user()->id);
        $data['two_step_verification'] = preference('two_step_verification');
        $data['documentVerification'] = DocumentVerification::where(['user_id' => auth()->user()->id, 'verification_type' => 'address'])->first(['file_id']);
    ...
    public function updatePersonalAddress(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $this->validate($request, [
                'address_file' => 'mimes:pdf,png,jpg,jpeg,gif,bmp|max:10000',
            ]);
