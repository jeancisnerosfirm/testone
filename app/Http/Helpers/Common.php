...
class Common
{
    public static $templateIds = ['deposit' => 23, 'payout' => 24, 'exchange' => 25, 'send' => 26, 'request' => 27, 'payment' => 28, 'crypto-exchange' => 34];
            ...
            if (filter_var($recipient, FILTER_VALIDATE_EMAIL))
            {
                ...
                else if ($type == 'payment')
                {
                    ...
                    if ($data->user_id) {               
                        ...
                        $message = str_replace('{user}', $data->transaction->phone, $message);
                    }
                    
                    $message = str_replace('{amount}', moneyFormat($data->fromCurrency->symbol, formatNumber($data->amount, $data->from_currency)), $message);
                    $message = str_replace('{from_wallet}', $data->fromCurrency->code, $message);
                    $message = str_replace('{to_wallet}', $data->toCurrency->code, $message);
                    $message = str_replace('{fee}', moneyFormat($data->fromCurrency->symbol, formatNumber($data->fee, $data->from_currency)), $message);
                }
