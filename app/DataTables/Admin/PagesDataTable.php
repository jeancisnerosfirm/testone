    ...
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($page) {
                $edit = $delete = '';
                $edit = (Common::has_permission(Auth::guard('admin')->user()->id, 'edit_page')) ? '<a href="' . url(Config::get('adminPrefix') . '/settings/page/edit/' . $page->id) . '"  class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                $delete = (Common::has_permission(Auth::guard('admin')->user()->id, 'delete_page')) ? '<a href="' . url(Config::get('adminPrefix') . '/settings/page/delete/' . $page->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>' : '';
                return $edit . $delete;
            })
