...
class RequestPaymentsDataTable extends DataTable
{
    public function ajax()
    {
        return datatables()
            ...
            ->addColumn('sender', function ($request_payment) {
                $sender = isset($request_payment->user->first_name) && !empty($request_payment->user->first_name) ? $request_payment->user->first_name . ' ' . $request_payment->user->last_name : "-";
            ...
            ->addColumn('action', function ($request_payment) {
                return (Common::has_permission(Auth::guard('admin')->user()->id, 'edit_request_payment')) ?
                    '<a href="' . url(Config::get('adminPrefix') . '/request_payments/edit/' . $request_payment->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
            })
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'receiver', 'name' => 'receiver.last_name', 'title' => 'Receiver', 'visible' => false])
