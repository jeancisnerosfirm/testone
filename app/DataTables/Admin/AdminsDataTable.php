    ...
    public function ajax()
    {
        return datatables()
            ...
            ->addColumn('action', function ($admin) {
                $edit = (Common::has_permission(Auth::guard('admin')->user()->id, 'edit_admin')) ? '<a href="' . url(Config::get('adminPrefix') . '/admin-user/edit/' . $admin->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                $delete = ($admin->id != 1 && Common::has_permission(Auth::guard('admin')->user()->id, 'delete_admin')) ? '<a href="' . url(Config::get('adminPrefix') . '/admin-user/delete/' . $admin->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>' : '';
                return $edit . $delete;
            })
