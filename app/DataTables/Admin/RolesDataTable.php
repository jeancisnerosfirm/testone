    ...
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($role) {
                $edit = (Common::has_permission(Auth::guard('admin')->user()->id, 'edit_role')) ? '<a href="' . url(Config::get('adminPrefix') . '/settings/edit_role/' . $role->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';

                $delete = (Common::has_permission(Auth::guard('admin')->user()->id, 'delete_role')) ? '<a href="' . url(Config::get('adminPrefix') . '/settings/delete_role/' . $role->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>' : '';
