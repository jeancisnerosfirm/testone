...
class CurrenciesDataTable extends DataTable
{
    public function ajax()
    {
        return datatables()
            ...
            ->addColumn('action', function ($currency) {
                ...
                $edit = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency')) ? '<a href="' . url(Config::get('adminPrefix').'/settings/edit_currency/' . $currency->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                $delete = (Common::has_permission(\Auth::guard('admin')->user()->id, 'delete_currency')) ? '<a href="' . url(Config::get('adminPrefix').'/settings/delete_currency/' . $currency->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>&nbsp;' : '';
                
                $feesLimit = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency') && $currency->status == 'Active') ? '<a href="' . url(Config::get('adminPrefix').'/settings/feeslimit/deposit/' . $currency->id) . '" class="btn btn-xs btn-success">
                <i class="glyphicon glyphicon-view">Fees</i></a>&nbsp;' : '';
                $pm = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency')) && $currency->status == 'Active'? '<a href="' . url(Config::get('adminPrefix').'/settings/payment-methods/'.$type.'/' . $currency->id) . '" class="btn btn-xs btn-primary">
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'type', 'name' => 'currencies.type', 'title' => 'Type'])
