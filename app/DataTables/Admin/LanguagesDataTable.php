    ...
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($language) {
                $edit = $delete = '';
                $edit = (Common::has_permission(Auth::guard('admin')->user()->id, 'edit_language')) ? '<a href="' . url(Config::get('adminPrefix') . '/settings/edit_language/' . $language->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';

                if ($language->deletable == 'No') {
                    $delete = '';
                } else {
                    $delete = (Common::has_permission(Auth::guard('admin')->user()->id, 'delete_language')) ? '<a href="' . url(Config::get('adminPrefix') . '/settings/delete_language/' . $language->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>' : '';
                }
                return $edit . $delete;
            })
