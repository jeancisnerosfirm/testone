    ...
    public function ajax() //don't use default dataTable() method
    {
        return datatables()
            ...
            ->addColumn('action', function ($country) {
                $edit = (Common::has_permission(Auth::guard('admin')->user()->id, 'edit_country')) ? '<a href="' . url(Config::get('adminPrefix') . '/settings/edit_country/' . $country->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                $delete = (Common::has_permission(Auth::guard('admin')->user()->id, 'delete_country')) ? '<a href="' . url(Config::get('adminPrefix') . '/settings/delete_country/' . $country->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>' : '';
